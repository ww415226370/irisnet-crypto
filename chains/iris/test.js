...
describe('CryPto iris test', function () {
  ...
  var chain = Irisnet.config.chain.iris; //测试热钱包相关交易
  ...
  function execute(tx) {
    ...
    var stdTx = builder.buildAndSignTx(tx, privateKey);
    console.log("======stdTx======");
    console.log((0, _stringify["default"])(stdTx.GetData())); // console.log("======待提交交易======");
  ...
  function simulate(tx) {
    ...
    var stdTx = builder.buildAndSignTx(tx);
